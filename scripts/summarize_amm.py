...
def main() -> int:
    ...
    final_window_table = summary.copy()
    final_window_table["gamma"] = final_window_table["gamma"].map(
        lambda x: f"{100*x:.0f}%"
    )
    final_window_table["accuracy"] = final_window_table.apply(
        lambda r: f"{100*r['accuracy_mean']:.2f} +/- {0 if pd.isna(r['accuracy_std']) else 100*r['accuracy_std']:.2f}",
        axis=1,
    )
    final_window_table["AggErr"] = final_window_table.apply(
        lambda r: f"{r['agg_err_mean']:.4e} +/- {0 if pd.isna(r['agg_err_std']) else r['agg_err_std']:.4e}",
        axis=1,
    )
    if {"amm_reject_rate", "benign_accept_rate"}.issubset(final_window_table.columns):
        final_window_table["AMMRejectRate"] = final_window_table[
            "amm_reject_rate"
        ].map(lambda x: f"{100*x:.2f}%")
        final_window_table["BenignAcceptRate"] = final_window_table[
            "benign_accept_rate"
        ].map(lambda x: f"{100*x:.2f}%")
        final_window_output = final_window_table[
            [
                "gamma",
                "mode_label",
                "accuracy",
        ...
        final_window_output = final_window_table[
            ["gamma", "mode_label", "accuracy", "AggErr", "rejected_clients"]
        ]
    final_window_output.to_csv(
        summary_dir / "final_window_summary.csv",
        index=False,
    )

    if cum_summary is None:
        raise ValueError(
            "Cumulative aggregate-error inputs are required to generate paper_table.csv"
        )

    no_verification = summary[summary["mode"] == "verification_off"][
        ["gamma", "accuracy_mean", "accuracy_std"]
    ].rename(
        columns={
            "accuracy_mean": "no_verification_mean",
            "accuracy_std": "no_verification_std",
        }
    )
    vmask = summary[summary["mode"] == "vmask"][
        ["gamma", "accuracy_mean", "accuracy_std"]
    ].rename(
        columns={
            "accuracy_mean": "vmask_mean",
            "accuracy_std": "vmask_std",
        }
    )
    paper = (
        no_verification.merge(
            cum_summary[
                [
                    "gamma",
                    "cumulative_agg_err_mean",
                    "cumulative_agg_err_std",
                ]
            ],
            on="gamma",
            how="inner",
    ...
    paper["gamma"] = paper["gamma"].map(lambda x: f"{100*x:.0f}%")
    paper["no_verification_accuracy"] = paper.apply(
        lambda r: f"{100*r['no_verification_mean']:.2f} +/- {100*r['no_verification_std']:.2f}",
        axis=1,
    )
    paper["AggErr_cum"] = paper.apply(
        lambda r: f"{r['cumulative_agg_err_mean']:.4f} +/- {r['cumulative_agg_err_std']:.4f}",
        axis=1,
    )
    paper["vmask_accuracy"] = paper.apply(
        lambda r: f"{100*r['vmask_mean']:.2f} +/- {100*r['vmask_std']:.2f}",
        axis=1,
    )
    paper = paper[
        [
            "gamma",
            "no_verification_accuracy",
            "AggErr_cum",
            "vmask_accuracy",
        ]
    ]
